...
export default function Sidebar({ role = 'ADMIN' }) {
  ...
  return (
    <aside className="w-64 bg-white border-r border-slate-200 min-h-[calc(100vh-4rem)] p-4 hidden md:flex flex-col justify-between flex-shrink-0">
      ...
      <div className="bg-slate-50 rounded-xl p-3 border border-slate-200/80">
        <div className="flex items-center space-x-2 text-sky-700 font-bold text-xs mb-0.5">
          <ShieldCheck className="w-3.5 h-3.5" />
          <span>Role Authorization</span>
        </div>
        <p className="text-[10px] text-slate-500 capitalize">{role.toLowerCase()} Access Level</p>
      </div>
    </aside>
  );
}
