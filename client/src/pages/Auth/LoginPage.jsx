...
import { HeartPulse, UserCheck, Lock, LogIn, KeyRound } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      setServerError(err.response?.data?.message || 'Login failed. Invalid email or password.');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="max-w-md mx-auto w-full px-4">
          <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-200 space-y-6">
            ...
            <div className="text-center space-y-2">
              ...
              <h1 className="text-2xl font-extrabold text-slate-900">User Portal Login</h1>
              <p className="text-xs text-slate-500">Access your donor or recipient portal account</p>
            </div>
            ...
            <div className="p-3 bg-sky-50 rounded-xl border border-sky-100 text-xs text-sky-900 space-y-1">
              ...
              {activeRole === 'DONOR' ? (
                <p>Email: <code className="bg-sky-100 px-1 rounded">john.david@example.com</code> | Pass: <code className="bg-sky-100 px-1 rounded">Password@123</code></p>
              ) : (
                <p>Email: <code className="bg-sky-100 px-1 rounded">arun.kumar@example.com</code> | Pass: <code className="bg-sky-100 px-1 rounded">Password@123</code></p>
              )}
            </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-3.5 rounded-xl font-bold text-sm text-white bg-sky-600 hover:bg-sky-700 shadow-md hover:shadow-lg transition-all disabled:opacity-50"
              >
                {loading ? 'Authenticating...' : `Log In to ${activeRole} Portal`}
              </button>
            </form>
